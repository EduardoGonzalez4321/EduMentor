  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      fetch('https://edumentor.onrender.com/api/users/userData', {
        ...
        body: JSON.stringify({
          jwt: jwt, // Incluye el token en el cuerpo
        }),
      })
        .then(response => {
          if (!response.ok) {
          ...
          return response.json(); // Parsea la respuesta como JSON en caso de éxito
        })
        .then(data => resolve(data)) // Resuelve con los datos del usuario en caso de éxito
        .catch(error => reject(error));
    } catch (error) {
      reject(new Error('Error en la solicitud'));
    }
