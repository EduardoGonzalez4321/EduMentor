...
// Ruta para el retrieve de los datos de usuario
router.route('/userData').post(async (req, res) => {
  try {
    const jwt = req.body.jwt; // Obtén el JWT del cuerpo de la solicitud

    let userData = await dataHandler.userData(jwt);

    console.log("DENTRO DE PERFIL: ", userData);
